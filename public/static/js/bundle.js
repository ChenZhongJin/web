...
function notify(_a) {
    ...
    var statusMap = ['danger', 'success', 'primary', 'info', 'warning', 'dark'];
...
function ajax(url, data, options) {
    ...
    $.ajax(__assign({ url: url, data: data, type: 'POST', success: function (resp) { notify(resp); }, error: function (xhr, statusText) {
            notify({ msg: "Request.Status:" + xhr.status + ",Error:" + statusText });
        } }, options));
}
